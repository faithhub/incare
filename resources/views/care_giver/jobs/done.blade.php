  ...
  <div class="col-lg-12">
    <div class="breadcrumb-content d-flex flex-wrap justify-content-between align-items-center">
      <div class="section-heading">
        <h2 class="sec__title">Done Job</h2>
    ...
    <div class="col-lg-12">
        <div class="billing-form-item">
            <div class="billing-title-wrap">
                <h3 class="widget-title pb-0">Done Job</h3>
            </div>
            @if ($jobs->count() == 0)
                <div class="text-center mb-3 mt-2">
                    <h2 style="color: black; font-family: Georgia, 'Times New Roman', Times, serif">No Done Job yet</h2>
                </div>                
            @else
                               ...
                               <tr>
                                ...
                                <td>
                                    @if ($job->job->status == 'Active')
                                        <span class="badge badge-success p-1">{{$job->job->status}}</span>                                            
                                    @elseif ($job->job->status == 'Blocked')
                                    <span class="badge badge-danger p-1">{{$job->job->status}}</span>                                          
                                    @elseif ($job->job->status == 'Pending')
                                    <span class="badge badge-warning p-1">{{$job->job->status}}</span>                                          
                                    @elseif ($job->job->status == 'Delivered')
                                    <span class="badge badge-success p-1">{{$job->job->status}}</span>                                           
                                    @endif
                                </td>
